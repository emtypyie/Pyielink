...
pub fn verify_password(record: &UserRecord, password: &str) -> bool {
    !record.pw_hash.is_empty()
        && hash_password(&record.pw_salt, password) == record.pw_hash
}
...
mod tests {
    ...
    fn password_hash_round_trip() {
        ...
        assert_eq!(h.len(), 64);
        let rec = UserRecord { pw_salt: salt, pw_hash: h, ..Default::default() };
        assert!(verify_password(&rec, "hunter2"));
        assert!(!verify_password(&rec, "hunter3"));
    }
